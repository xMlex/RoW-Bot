...
public final class StatsUtil {
    ...
    public static CharSequence getMemUsage() {
        ...
        list.append("     Non-Allocated: ...... ").append((int) nonAllocatedMem).append(" KB (")
                .append(Math.round(nonAllocatedMem / maxMem * 1000000.0D) / 10000.0D).append("%)").append("\n");
    ...
    public static CharSequence getMemUsageAdm() {
        ...
        list.append("<tr><td>     Выделено: .......... ").append((int) allocatedMem).append(" KB (")
                .append(Math.round(allocatedMem / maxMem * 1000000.0D) / 10000.0D).append("%)").append("</td></tr>");
        list.append("<tr><td>     Нераспределенно: ...... ").append((int) nonAllocatedMem).append(" MB (")
                .append(Math.round(nonAllocatedMem / maxMem * 1000000.0D) / 10000.0D).append("%)").append("</td></tr>");
        ...
        list.append("<tr><td>     Используется: ............... ").append((int) usedMem).append(" MB (")
                .append(Math.round(usedMem / maxMem * 1000000.0D) / 10000.0D).append("%)").append("</td></tr>");
        list.append("<tr><td>     Не используется (cached): .... ").append((int) cachedMem).append(" MB (")
                .append(Math.round(cachedMem / maxMem * 1000000.0D) / 10000.0D).append("%)").append("</td></tr>");
        list.append("<tr><td>Полезная памяти: ........... ").append((int) useableMem).append(" MB (")
                .append(Math.round(useableMem / maxMem * 1000000.0D) / 10000.0D).append("%)").append("</td></tr>");
    ...
    public static CharSequence getThreadStats(boolean lockedMonitors, boolean lockedSynchronizers, boolean stackTrace) {
        ...
        for (ThreadInfo info : threadMXbean.dumpAllThreads(lockedMonitors, lockedSynchronizers)) {
            list.append("Thread #").append(info.getThreadId()).append(" (").append(info.getThreadName()).append(")")
                    .append("\n");
            list.append("=================================================\n");
