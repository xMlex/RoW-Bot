...
public class FiltredPreparedStatement implements FiltredStatementInterface {
    ...
    public void setString(int index, String val) throws SQLException {
        /*
         * if(val != null && val.length() > 0) { TextBuilder tb =
         * TextBuilder.newInstance();
         *
         * char[] arr = val.toCharArray(); for(int i = 0; i < arr.length; i++)
         * // Защита от инъекций, экранируем 34 " 39 ' 92 \ if(arr[i] == 34 ||
         * arr[i] == 39 || arr[i] == 92) tb.append(_92).append(arr[i]); else
         * tb.append(arr[i]); val = tb.toString(); TextBuilder.recycle(tb); }
         */
