...
public class StringDES implements TStringCrypt {
    protected static Logger _log = Logger.getLogger(StringDES.class.getName());

    private static StringDES _instance = new StringDES();

    public static StringDES getInstance() {
        return _instance;
    }
    ...
    private static void myuwSplit(long w, short[] bytes, int index) {
        ...
        bytes[index + 1] = (short) Math
                .round((w - bytes[index + 3] * 16777216L - bytes[index + 2] * 65536L) / 256L);
        bytes[index] = (short) Math
                .round((w - bytes[index + 3] * 16777216L - bytes[index + 2] * 65536L - bytes[index + 1] * 256L));
    }

    private static long bytesToInt(short[] bytes, int index) {
        return bytes[index] + bytes[index + 1] * 256L + bytes[index + 2] * 65536L + bytes[index + 3]
                * 16777216L;
    }
}